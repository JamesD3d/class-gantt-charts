# Housing Project
Subtitle: 10-Class Period Project Outline
Columns: Day 1, Day 2, Day 3, Day 4, Day 5, Day 6, Day 7, Day 8, Day 9, Day 10

## Initial Research
Points: 4 pts
Start: 1
Duration: 1
Label: Research Phase Existing Homes
Tooltip: Housing Research
- Use web sites like Zillow, Redfin & Relitor
- Complete the Google Document
- Complete the assignment Reflection

## Continued Research 
Points: 4 pts
Start: 2
Duration: 1
Label: Floor Plans 
Tooltip: Use Houseplans.com , architecturaldesigns.com, or floorplans.com
- Complete the Google Document
- Complete the assignment reflection

## Select Your Plan
Points: 4 pts
Start: 2
Duration: 2
Label: Use Houseplans.com , .architecturaldesigns.com, or www.floorplans.com
Tooltip: Select a floor plan to model based on the assigned constraints. 
- Ensure your floor plan meets the criteria. 
- Complete the Google Document
- Complete the assignment reflection

## CAD
Points: 4 pts
Start: 3
Duration: 2
Label:CAD
Tooltip: Use Onshape & screen shot to import images into an Onshape Project file
- Include all floors in a separate part studio and elevation images of the roof 
- Create a drawing of the complete sketch
- include all doors and windows. 

## CAD
Points: 4 pts
Start: 5
Duration: 3
Label:CAD to SOLID
Tooltip: Sketch to Create a 3d model of your house 
- ensure all doors and windows are present in the final solid 
- wall height is 9 ft
- wall thickness is 8 in
- U\use the elevation images to create a basic Roof Profile
- add a floor to your project.
- Scale your project so that its widest point is no more than 200. mm   

## CAD
Points: 4 pts
Start: 7
Duration: 1
Label: Cut out and label each room. 
Tooltip: Ensure everything is scaled correctly. 
- ensure all doors and windows are present in the final solid
- Use the elevation images to create a basic Roof Profile
- add a floor to your project.  

## Energy 
Points: 4 pts
Start: 8
Duration: 1
Label: Complete an Energy Audit  
- Complete an Energy Audit
- Complete the Google Document
- Complete the assignment reflection

## Financial 
Points: 4 pts
Start: 8
Duration: 2
Label: Complete a Financial Audit   
- Complete a FInancial Audit 
- Complete the Google Document
- Complete the assignment reflection

## Final
Points: 4 pts
Start: 9
Duration: 2
Label: Assemble your model with a detailed Report for your home in your zipcode  
- Complete the assignment reflection
